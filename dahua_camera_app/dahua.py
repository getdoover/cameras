...
POSITION_MATCHER = re.compile(r"status\.Postion\[0]=(?P<PanPos>(\d|\.)+)\n"
...
class DahuaClient:
    ...
    async def get_status(self):
        try:
            ...
            logging.exception("Failed to get camera status.", exc_info=e)
            return False
    ...
    async def adjust_manual_zoom(self, zoom: float, focus: float = None, channel: int = 1):
        """
        ...
        return await self.get(
            f"/cgi-bin/devVideoInput.cgi?action=adjustFocus&focus={focus}&zoom={zoom}&channel={channel}", True
        )
    ...
    async def auto_focus(self, channel: int = 1):
        return await self.get(f"/cgi-bin/devVideoInput.cgi?action=autoFocus&channel={channel}", verify_ok=True)
    ...
    async def get_ptz_position(self, channel: int = 1):
        ...
        return (float(data.get("status.Postion[0]")),
    ...
    async def stop_continuous_ptz(self, channel: int = 1, delay: int = None):
        ...
        return await self.get(f"/cgi-bin/ptz.cgi?action=stop&code=Continuously&channel={channel}&arg1=0&arg2=0&arg3=0&arg4=0")
    ...
    async def continuous_ptz(self, pan, tilt, zoom, timeout, channel: int = 1):
        ...
        self.continuous_ptz_stop_task = asyncio.create_task(self.stop_continuous_ptz(channel, timeout))
    ...
    async def create_preset(self, name, channel: int = 1):
        ...
        await self.get(
            f"/cgi-bin/ptz.cgi?action=setPreset&channel={channel}&arg1={new_id}&arg2={name}", verify_ok=True
        )
    ...
    async def stream_events(self, callback, events: list, channel: int = 1, heartbeat: int = 5):
        """
