...
def test_ensure_full_frame_grid():
    from camera_app.clients.hikvision import HikvisionClient

    def body(grid):
        return f"<MotionDetection><enabled>true</enabled><gridMap>{grid}</gridMap></MotionDetection>"

    # A grid the camera has never had configured comes back empty or all-zero, and the
    # rule then matches nothing - motion is "on" and never fires. Fill it.
    assert "<gridMap>ffffff</gridMap>" in HikvisionClient._ensure_full_frame_grid(
        body("000000")
    )
    # Empty: no length to preserve, so the 22x18 grid's full 108 hex chars.
    assert "<gridMap>" + "f" * 108 in HikvisionClient._ensure_full_frame_grid(body(""))
    # A grid with cells already set is somebody's mask - leave it exactly as drawn.
    assert HikvisionClient._ensure_full_frame_grid(body("ff0000")) == body("ff0000")
    # No grid element at all (other firmware): nothing to do, don't invent one.
    assert "gridMap" not in HikvisionClient._ensure_full_frame_grid("<MotionDetection/>")


def test_deepinview_day_night_triggers():
    """Day is driven by VMD, night by the classified perimeter rule - never both."""
    import asyncio
    import types
    ...
    def route(event, night):
        seen = []
        cam = HikvisionDeepinViewCamera.__new__(HikvisionDeepinViewCamera)
        cam.config = types.SimpleNamespace(is_night=lambda: night)
        cam.on_motion_event_callback = seen.append
        asyncio.run(cam.on_cam_event(event))
        return seen

    vmd = {"eventType": "VMD", "eventState": "active"}
    intrusion = {
        "eventType": "fielddetection",
        "eventState": "active",
        "DetectionRegionList.DetectionRegionEntry.detectionTarget": "human",
    ...
    # Day: motion is the trigger, unclassified on purpose - the cloud classifies it.
    (event,) = route(vmd, night=False)
    assert event.type is MotionDetectEventType.motion
    # ...and the perimeter rule is ignored, so one person crossing isn't captured twice
    # (this holds even if the camera rejected its night-only arming schedule).
    assert route(intrusion, night=False) == []

    # Night: the reverse. Only a classified target should be able to wake a siren.
    assert route(vmd, night=True) == []
    (event,) = route(intrusion, night=True)
    assert event.type is MotionDetectEventType.person

    # "inactive" is the clear, not a trigger.
    assert route({"eventType": "VMD", "eventState": "inactive"}, night=False) == []


def test_unclassified_motion_capture():
    """Plain motion captures a frame on DeepinView, and nothing anywhere else."""
    import asyncio
    import types
    from datetime import timedelta

    from camera_app.application import (
        MOTION_SNAPSHOT_MIN_INTERVAL_SEC,
        REASON_MOTION,
        CameraApplication,
    )

    def make_app(daytime_capture, allowed=True):
        app = CameraApplication.__new__(CameraApplication)
        app.engine = types.SimpleNamespace(daytime_motion_capture=daytime_capture)
        app.config = types.SimpleNamespace(motion_snapshot_allowed=lambda: allowed)
        app.snapshot_running = None
        app._last_motion_snapshot_at = None
        app.captured = []
        app.lock_snapshot_and_run = lambda reason: _record(app, reason)
        return app

    async def _record(app, reason):
        app.captured.append(reason)

    # An engine that doesn't drive the day off motion is unchanged: not actionable.
    app = make_app(daytime_capture=False)
    asyncio.run(app.on_unclassified_motion())
    assert app.captured == []

    # DeepinView: the frame is captured, marked with the unclassified reason.
    app = make_app(daytime_capture=True)
    asyncio.run(app.on_unclassified_motion())
    assert app.captured == [REASON_MOTION]

    # Motion re-fires while something moves; the cooldown stops one car becoming a
    # dozen uploads of the same vehicle.
    asyncio.run(app.on_unclassified_motion())
    assert app.captured == [REASON_MOTION]
    app._last_motion_snapshot_at -= timedelta(seconds=MOTION_SNAPSHOT_MIN_INTERVAL_SEC)
    asyncio.run(app.on_unclassified_motion())
    assert app.captured == [REASON_MOTION, REASON_MOTION]

    # Outside the motion-snapshot window, no picture.
    app = make_app(daytime_capture=True, allowed=False)
    asyncio.run(app.on_unclassified_motion())
    assert app.captured == []


def test_snapshot_func_still_override():
    """Motion capture forces a still even on a camera configured for video."""
    import types

    from camera_app.app_config import Mode
    from camera_app.engines.base import CameraBase

    cam = CameraBase.__new__(CameraBase)
    cam.config = types.SimpleNamespace(
        snapshot=types.SimpleNamespace(mode=types.SimpleNamespace(value=Mode.video))
    )
    assert cam.snapshot_func()[0] == cam.get_video_snapshot
    assert cam.snapshot_func()[1] == "mp4"
    # The cloud can't analyse an mp4, so motion asks for a frame regardless.
    assert cam.snapshot_func(still=True)[0] == cam.get_still_snapshot
    assert cam.snapshot_func(still=True)[1] == "jpg"

    cam.config.snapshot.mode.value = Mode.image
    assert cam.snapshot_func()[1] == "jpg"
